...
fun all_except_option (s : string, xs : string list) =
  let
      fun generate_help (target : string, acc : string list, left : string list) =
        (*
            target : judge the string in that list is the same as given string s
            acc    : for each element in *test* string list, not the same with s
        ...
        then SOME(acc @ left)
                 ...
                 [] => NONE
               | x::left' => generate_help (x, acc @ (target::[]), left')
  in
      case xs of
          [] => NONE
...
fun similar_names (xs : string list list, {first=x, middle=y, last=z}) =
  let
      ...
      fun generate_same_name (first_names : string list, produce) =
        case first_names of
            [] => produce
          | head::tail => generate_same_name (tail,
                                              produce @ ({first=head,middle=y,last=z})::[])
  in
